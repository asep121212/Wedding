...
<body data-email="user@example.com" data-password="12345678" data-url="https://weddingarinova.my.id/sandbox/">
    ...
    <main class="scrollspy-example text-light" data-bs-spy="scroll" data-bs-target="#navbar-example2"
        ...
        " id="tanggal">
            <div class="container">
                <div class="text-center">
                    <h1 class="font-estetik py-3" style="font-size: 2rem">
                        Waktu Menuju Acara
                    </h1>
                    @foreach($dates as $date)

                    <div class="border rounded-pill shadow py-2 px-4 mx-2 mb-4">
                        <div class="row justify-content-center" data-waktu="2023-10-08 00:00:00" id="tampilan-waktu">
                            <div class="col-3 p-1">
                                <h2 class="d-inline m-0 p-0" id="hari">{{ $date->day }}</h2>
                                <small class="ms-1 me-0 my-0 p-0 d-inline">Hari</small>
                            </div>
                            <div class="col-3 p-1">
                                <h2 class="d-inline m-0 p-0" id="jam">{{ $date->hour }}</h2>
                                <small class="ms-1 me-0 my-0 p-0 d-inline">Jam</small>
                            </div>
                            <div class="col-3 p-1">
                                <h2 class="d-inline m-0 p-0" id="menit">{{ $date->minute }}</h2>
                                <small class="ms-1 me-0 my-0 p-0 d-inline">Menit</small>
                            </div>
                            <div class="col-3 p-1">
                                <h2 class="d-inline m-0 p-0" id="detik">{{ $date->second }}</h2>
                    ...
                    <div class="overflow-x-hidden">
                        ...
                        <div class="py-2" data-aos="fade-left" data-aos-duration="1500">
                            <h1 class="font-estetik" style="font-size: 2rem">{{ $timeline->name }}</h1>
                            <p>Tangal {{ $timeline->date }}</p>
                            <p>{{ $timeline->story }}</p>
                        </div>
                        @endforeach
                        ...
                        <div class="py-2" data-aos="fade-up" data-aos-duration="1500">
                            <a href="https://maps.app.goo.gl/C8SUbovJQdhaZhrr7" target="_blank"
                            ...
                            <p class="mb-0 mt-1 mx-1 pb-4" style="font-size: 0.9rem">
                                Desa Negara Saka RT 03 Kec. Negeri Katon Kab.Pesawaran
                            </p>
                        </div>
    ...
    " id="galeri">
            <div class="container pb-4 pt-5">
                <div class="card-body border rounded-4 shadow p-4" style="background-color: #222;">
                    ...
                    <div id="carouselExampleIndicators" data-aos="fade-up" data-aos-duration="1500"
                        ...
                        <div class="carousel-inner rounded-4 shadow-sm" style="max-height: 350px;">
                            @foreach ($galleries as $key => $gallery)
                            <div class="carousel-item {{ $key == 0 ? 'active' : '' }}">
                                <img src="{{ asset('storage/' . $gallery->image) }}" class="d-block w-100"
                                    onclick="modalFoto(this)" alt="{{ $gallery->description }}"
        ...
        <div class="container">
            <div class="py-4">
                <div class="text-center">
                    <h1 class="font-estetik mt-0 mb-3" style="font-size: 3rem">
                        Love Gift
                    </h1>
                    ...
                    <div class="overflow-x-hidden">
                        <div class="row justify-content-center">
                            ...
                            <div class="col-12 card-body border rounded-4 shadow p-3 m-3" style="max-width: 25rem"
                                ...
                                <img src="{{ asset('storage/' . $card->image) }}" class="img-fluid rounded" width="150"
                                    alt="{{ $card->title }}" />
                                <p class="card-text mt-3 mb-0" style="font-size: 0.9rem">
                                    No. Rekening {{ $card->text }}
                                </p>
                                <p class="card-text" style="font-size: 0.9rem">
                                    a.n {{ $card->title }}
                                </p>
                                <button class="btn btn-light btn-sm rounded-4" data-nomer="{{ $card->text }}"
        ...
        <section class="m-0 p-0" id="ucapan">
            <div class="container">
                <div class="card-body border rounded-4 shadow p-3">
                    <h1 class="font-estetik text-center mb-3" style="font-size: 3rem">
                        Ucapan & Doa
                    </h1>
                    ...
                    <form id="commentForm" method="POST" action="{{ route('comments.store') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control shadow-sm" id="formnama" name="nama"
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control shadow-sm" id="formpesan" name="pesan" rows="4"
                                placeholder="Tulis Ucapan & Doa" required></textarea>
                        </div>
                        <div class="d-grid mb-2">
                            <button type="submit" class="btn btn-primary btn-sm rounded-4 shadow">
        ...
        ">
            <div class="container">
                <div class="text-center">
                    ...
                    <hr class="mt-3 mb-2" />
                    <div class="row align-items-center justify-content-between flex-column flex-sm-row"></div>
                </div>
            </div>
        </div>
    </main>
    ...
    <button type="button" id="tombol-musik" style="display: none" class="btn btn-light btn-sm rounded-circle btn-music"
        onclick="play(this)" data-status="true" data-url="assets/music/sound2.mp3">
